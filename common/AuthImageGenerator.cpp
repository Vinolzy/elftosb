...
int AuthImageGenerator::createImageLpcSboot(const configuration &conf)
{
	...
	else
	{
		...
		signatureSizeInBytes = rsa_pub_key_ctx->len;
		...
		if (conf.mainCertPrivateKeyFile == "-1")
		{
			Log::log(Logger::INFO2, "\tImage will be generated to be signed by external tool.\n", step);
			Log::log(Logger::INFO2, "\tSuccess");
		}
		else
		{
			...
			auto len = rsa_priv_key_ctx->len;
			if (len != signatureSizeInBytes)
			{
				Log::log(Logger::ERROR, "Private and Public key size in bytes are not corresponding (%d vs %d).\n", len, signatureSizeInBytes);
				cleanUpLpcSboot(userKey, keyStore, imageFile, usedRootCrtFile, outputFile, trustZonePresetFile, buffer, signature, trustCrt, rsa_priv_key_ctx);
				return FAIL_RETURN;
			}
		}
	...
	*(uint32_t *)(buffer + 0x34) = imageLinkAddress;
	...
	if (imageType != kSKBOOT_ImageTypeXipPlainCrc && imageType != kSKBOOT_ImageTypePlainCrc)
	{
		...
		{
			Log::log(Logger::INFO2, "%02x", digest[i]);
		}
		Log::log(Logger::INFO2, "\n");
		step++;
		if (conf.mainCertPrivateKeyFile == "-1") {
			Log::log(Logger::INFO2, "%d. Image signature based on previous hash value need to placed on the end of generated image.\n", step);
			if (imageType == kSKBOOT_ImageTypePlainSigned || imageType == kSKBOOT_ImageTypeEncryptedSigned)
			{
				Log::log(Logger::INFO2, "\tFor hash calculation needs to be skipped %d bytes starting on offset %#x.\n\tThis data containd HMAC of image header and are not part of image digital signature.\n", sizeof(_headerMac_t), HMACHEADEROFSET);
			}
			if (imageType == kSKBOOT_ImageTypePlainSignedKeyStoreIncluded || imageType == kSKBOOT_ImageTypeEncryptedSignedKeyStoreIncluded) {			
			
				Log::log(Logger::INFO2, "\tFor hash calculation needs to be skipped %d bytes starting on offset %#x.\n\tThis data contains HMAC of image header + Key Store and are not part of image digital signature.\n", sizeof(_headerMac_t) + keyStoreSize, HMACHEADEROFSET);
			}
			if (certCount == 0)
				Log::log(Logger::INFO2, "\tCertificate used for signature verification: %s\n", usedRootCertPath.c_str());
			else
				Log::log(Logger::INFO2, "\tCertificate used for signature verification: %s\n", conf.chainCertFiles[certCount].c_str());
			Log::log(Logger::INFO2, "\tExpected signature size in bytes: %d.\n", signatureSizeInBytes);
